...
def log_in(bot_id, login, password, dur_speed_from, dur_speed_to, typing_speed_from, typing_speed_to):
    ...
    sleep(5)
    try:
        x, y = pag.locateCenterOnScreen(r"img\other\simple.png", grayscale=False)
        pag.moveTo(x, y, duration=(random.uniform(dur_speed_from, dur_speed_to)))
        pag.click(x, y)
        x, y = pag.locateCenterOnScreen(r"img\auth\login_box.png", grayscale=False)
        pag.moveTo(x, y, duration=(random.uniform(dur_speed_from, dur_speed_to)))
        pag.click(x, y)
        ...
        pag.click(x+300, y)
        x, y = pag.locateCenterOnScreen(r"img\auth\password_box.png", grayscale=False)
        pag.moveTo(x, y, duration=(random.uniform(dur_speed_from, dur_speed_to)))
        pag.click(x, y)
